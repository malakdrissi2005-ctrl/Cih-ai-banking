...
def login(payload: LoginRequest, db_path: Optional[str] = Depends(get_auth_db_path)) -> LoginResponse:
    ...
    user = session_manager.verify_credentials(payload.username, payload.password, db_path=db_path)
    if user is None:
...
def check_session(
    ...
    session = session_manager.get_valid_session(session_id, db_path=db_path)
    if session is None:
